...
make_stage_splash :: proc() -> Stage {
	...
	stage.draw = proc(scenario: Outline) {
		...
		font_made := scenario.fonts.title.sm
		...
		position_jam := rl.Vector2{ (SCREEN - measure_jam.x) / 2, (SCREEN - measure_jam.y) / 2 + f32(font_made.baseSize) }
		position_made := rl.Vector2{ (SCREEN - measure_made.x) / 2, (SCREEN - measure_made.y) / 2 - f32(font_made.baseSize) }
